...
- **a)** A particle pair’s energy state in a QDF (different GS states or sublevels of a GS, or see Sec. 3 of the published article at https://doi.org/10.1016/j.simpa.2024.100703), <br>
...
- **c)** the expected transformation of fields (ES $\longleftrightarrow$ GS) and their $⟨{\frak M}(\cal{P}, \psi_{ij})⟩$, as in Sec. 3 of the published article.
  ...
  On the other hand, any dataset sample in the Qusole model to process from one scale to another by its simulator denote:
...
- **b)** An entangled state between the dataset values determined after a QDF-based QAI
dataset processing relative to an event or state from (a.), prior to its transform into a QDF as a TS:
