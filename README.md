...
[![Open as HTML/JS BI](https://img.shields.io/badge/BI-Visualize_Data_in_HTML_JS-%23FBBC04?logo=zaim&logoColor=%23FBBC04
)](./assets/datasets/PowerBI_QDF.pbix)
...
A Qusole model examines  ($\pm$) products impacting societies, and provides solutions against the negative impacts, prior to occurrence through its simulation program as apart of the QFLCA project.
...
flowchart TD
    ...
    classDef macro fill:#161b22,stroke:#30363d,stroke-width:2px,color:#c9d1d9
    classDef core fill:#0d1117,stroke:#58a6ff,stroke-width:3px,color:#fff
    classDef optimal fill:#238636,stroke:#fff,color:#fff,stroke-width:2px
    classDef indeterminate fill:#f85149,stroke:#fff,color:#fff,stroke-width:2px
    classDef ids fill:#a277ff,stroke:#fff,color:#000,stroke-width:2px
    classDef final fill:#0f1115,stroke:#f0e68c,stroke-width:3px,color:#f0e68c


    A[Macroscopic QuSole Dataset\nImport: Orig C, κ, ρ]:::macro --> B(QF-LCA Tri-Core Validator\nSubset Boundary N):::core
    
    B --> C{Thermodynamic\nGround State Evaluation}:::core
    
    C -->|P ≥ 0.67 & Stability ≤ 2.05| D[Safe Ground State\nParameters Preserved]:::optimal
    C -->|P < 0.67 or Stability > 2.05| E[Indeterminate State\nTrigger IDS Rerouting]:::indeterminate
    
    subgraph IDS [Intelligent Decision Simulator]
        E --> F[1. Microscopic Recalibration\nρ_rec = 2N / 3_N-1]:::ids
        F --> G[2. Ground State Stability\nκ_rec = √1.95 / ρ_rec]:::ids
        G --> H[3. Probability Focusing\nShift D_rec for P ≥ 0.68]:::ids
    end
...
`const stability = (K * K) * rho;` "the validator calculates the boundary condition using the `(K * K) * rho` formula to determine if the state is stable. If the result exceeds the `2.05` threshold as `stability > 2.05`, the system returns a verdict of `LOSS` and triggers the IDS rerouting protocol."
...
The QFLC simulation program as part of QFCM (figure above) analyzes the measurement outcome probability ($\cal P$) data from datasets generated by Quantum Double-field (QDF) Circuits and Macroscopic Event from different labs (state samples like e.g., cancer cells and healthy cells). The datasets are compared between ES and GS states as a $\cal P$ indicator generated for measurement samples. Small dataset samples on a quantum scale denote:

- **a)** A particle pair’s energy state in a QDF (different GS states or sublevels of a GS, or see Sec. 3 of the published article at https://doi.org/10.1016/j.simpa.2024.100703), 
...
For more detailed illustration and the QFLCS itself, visit https://github.com/SoftwareImpacts/SIMPAC-2024-159. On the other hand, any dataset sample in the Qusole model to process from one scale to another by its simulator denote: <br>
...
[Link to QSole Codes and Datasets supplementary file.](<root/Codes_Datasets.md>)
